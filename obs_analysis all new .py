...
USER_CONFIG = {
    "env_id": "Humanoid-v5",       
    "epsilon": 0.03,               # 控制图 1, 2, 3 的全局扰动大小
    "fig4_epsilon": 0.05,          # 🌟 图 4 (视觉隐蔽性条形码) 的独立扰动大小
...
class ATNGenerator(nn.Module):
    ...
    def forward(self, obs, epsilon):
        # 严格遵循数学逻辑：由激活层输出给加法器前，先乘以 epsilon
...
# ===================== [3. 核心时机数据采集逻辑] =====================
def collect_combined_analysis_data(env_id, model, atn, shadow_models, device, epsilon):
    env = gym.make(env_id)
...
# ---------------- 图 3：状态空间流形偏移 (三子图) ----------------
    ...
    # 🌟 2. 移除 sharex=True, sharey=True，让每张图拥有独立边框
...
if __name__ == "__main__":
    ...
    print(f"✅ 成功加载 {len(shadow_models)} 个影子探测模型。")
