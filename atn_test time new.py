...
# ================= 2. 核心黑盒评估逻辑 =================
def run_evaluation(model_name, target_model, shadow_ensemble, atn, epsilon, episodes=50, device="cpu", video_root=None, baseline_reward=None):
    ...
    if video_root and epsilon > 0:
        current_video_dir = os.path.join(video_root, f"{model_name}_eps_{epsilon:.3f}_blackbox")
    ...
    for ep_idx in pbar:
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                # ================= [核心修正: 纯黑盒时机判定] =================
                # 2. 后台推演：利用本地【影子模型阵列】计算动作偏差，绝不偷看黑盒目标模型！
                shadow_mses = []
                for shadow_m in shadow_ensemble:
                    # 影子模型输出的是张量，直接在 GPU 上算 MSE，速度更快
            ...
            # ================= 稀疏攻击时机判定逻辑 =================
            # 默认执行干净动作 (不攻击)
            executed_obs_np = obs_t.cpu().numpy() 
            applied_rpr = 0.0               
            ...
            if ep_steps < 100:
                ...
                # 触发条件：当前状态的影子期望偏差 > 平稳期均值
                if current_mse > mu and epsilon > 0:
                    # 决定攻击：将对抗状态作为实际输入发给黑盒
                    executed_obs_np = adv_obs_np
                    ep_attack_count += 1
            ...
            # ================= [黑盒执行] =================
            # 3. 目标模型（黑盒）仅仅作为一个被动的响应者，只接受我们最终决定发送的观测值
...
if __name__ == "__main__":
    ...
    print(f"🚀 初始化【严格黑盒】评估系统... 设备: {device}")
    
    res_dir = "./atn_timing_results1"
    ...
    base = "translearning/"
    shadow_paths = [
        base + f"model/shadow_humanoid_SAC_model_{k}.pth" for k in ["50K", "200K", "500K"]
    ] + [
        base + f"model/shadow_humanoid_TQC_model_{k}.pth" for k in ["50K", "200K", "500K"]
    ]
    
    print("📦 正在挂载影子模型阵列作为探测器...")
    shadow_ensemble = []
    ...
    csv_file = os.path.join(res_dir, "blackbox_full_metrics1.csv")
    with open(csv_file, mode='w', newline='') as f:
        ...
        for name, path in target_configs.items():
            ...
            print(f" 🔍 正在校准基准分数...")
            base_data = run_evaluation(name, target_model, shadow_ensemble, atn, 0.0, episodes=20, device=device)
            baseline_rew = base_data['rew']
            print(f" 📊 黑盒基准奖励: {baseline_rew:.2f}")
            
            for eps in epsilons:
                res = run_evaluation(name, target_model, shadow_ensemble, atn, eps, episodes=50, device=device, 
                ...
                print(f" ✅ Eps {eps:.3f} | Rew: {res['rew']:.0f} | ASR: {res['asr']:.1f}% | 攻击频率: {res['atk_rate']:.1f}%")

    print(f"\n✨ 黑盒评估流执行完毕！结果已存档至: {csv_file}")